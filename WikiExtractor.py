...
Options:
  ...
  -e, --escaped			: preserve (double)escaped HTML &amp;nbsp;
...
##
# Whether to transform escaped HTML into plaintext
# &amp;nbsp; -> &nbsp; -> ' '
keepEscaped = False #True
...
discardElements = set([
		...
		'ref', 'references', 'imagemap', 'source'
		])
# 'img',
...
bad_images = ("desc", "Converted", "Cc", "Wiki", "Commons-logo", "important", "go-forward", "PD-icon", "GNU", "lightblue", "right", "trademark", ".js", "Disambig", "-logo")
# imagePattern = re.compile(r'File:([a-zA-Z0-9_\\-\\s]*?)(.jpg|.bmp|.png|.gif)') #(\w*)
good_images = list()
segment_images = list()
image="" # ONE!
...
def clean(text):
	...
	################ Process HTML ###############
	text = unescape(text) #ONCE OK IF &amp; b -> a and b
	# turn into HTML	
	if not keepEscaped:	
		# text = unescape(text) #ONCE NOT OK a &amp; b -> a & b ERROR
		text = unescape(text)
	# ^^ do it again (&amp;nbsp;)
	
	# text = re.sub(r'<(\d+)','&lt;\\1',text)
	# text = re.sub(r'>(\d+)','&gt;\\1',text)
	# text = re.sub(r'(\d+)>','\\1&gt;',text) # <1> <2> 
	# Collect spans
	...
	for line in text.split('\n'):
		...
		if m:
			...
			if title and title[-1] not in '!?':
				...
				#title += '.'
			headers[lev] = title #KF: don't repeat title
			...
			emptySection = True
			continue
		# Handle page title # ++WHAT's that?++
		if line.startswith('++'):
			title = line[2:-2]
			if title:
				if title[-1] not in '!?':
					title += '.'
				page.append("<h5>%s</h5>" % title)
		# handle lists
		elif line[0] in '*#:;':
			if keepSections:
...
def main():
	global keepLinks,keepEscaped, keepSections, prefix, acceptedNamespaces
	script_name = os.path.basename(sys.argv[0])

	try:
		long_opts = ['help', 'compress', 'bytes=', 'basename=', 'links', 'ns=', 'sections','escaped', 'output=', 'version']
		opts, args = getopt.gnu_getopt(sys.argv[1:], 'cb:hln:o:B:sev', long_opts)
	except getopt.GetoptError,e :
		print >> sys.stderr, 'EEE'
		print >> sys.stderr, e
	...
	if len(args) > 0:
		print >> sys.stderr, 'EXTRA OPTIONS: ', args
		show_usage(script_name)
		sys.exit(4)
